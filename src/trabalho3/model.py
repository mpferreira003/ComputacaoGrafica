...
class Model():
    ...
    def __init__(self, vertice_inicial, qtd_vertices, texture_id, pos=np.zeros(3),
             scale=np.ones(3), rotation=[(np.zeros(3), 0)], centroid=np.zeros(3),
             normal_scale=1, is_outdoor=True,
             ka=0.3, kd=1.0, ks=0.8, ns=64.0):  # Parâmetros de material adicionados
        ...
        self.ka = ka  # Coeficiente ambiente
